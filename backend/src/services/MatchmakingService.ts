...
export class MatchmakingService {
  ...
  start(): void {
    ...
    console.log(`🎮 Starting matchmaking loop (interval: ${config.match.matchmakingIntervalMs}ms)`);
  ...
  async joinQueue(socket: AuthenticatedSocket): Promise<void> {
    ...
    if (existingMatchId) {
      ...
      console.log(`   ✅ Previous match ${existingMatchId} is finished, allowing queue join`);
    }
  ...
  private async createMatch(
    ...
    console.log(`   ${player1.username} (${player1.elo}) vs ${player2.username} (${player2.elo})`);
  ...
  async handleDisconnect(socket: AuthenticatedSocket): Promise<void> {
    ...
    // Check if in match
    const matchId = socket.data.currentMatchId || await redisService.getUserMatchId(user.id);
    ...
    if (matchId) {
      ...
      if (match && match.status === 'active') {
        ...
        const winnerId = match.player1.id === user.id
          ? match.player2.id
          : match.player1.id;
