...
const ProfilePage: React.FC = () => {
  ...
  const customImage =
    profile?.avatar_url || user?.user_metadata?.profile_image || profileImage;
  ...
  const handleUsernameUpdate = async () => {
    ...
    try {
      ...
      if (existingUser && existingUser.username !== username) {
        alert(
          `Username "${trimmedUsername}" is already taken. Please choose another one.`
        );
        return;
      }
      ...
      <div className="max-w-7xl mx-auto px-4 relative z-10">
        ...
        <div className="flex flex-col md:flex-row gap-8 items-end mb-16 pb-8 border-b border-stone-800">
          ...
          <div className="flex-1">
            ...
            <p className="text-stone-500 text-sm font-bold uppercase tracking-wide">
              {user?.email || "user@deadlock.dev"}
            </p>
          </div>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              {[
                ...
                },
                {
                  ...
                  value: loading
                    ? "..."
                    : `${profile?.win_rate?.toFixed(1) || "0.0"}%`,
